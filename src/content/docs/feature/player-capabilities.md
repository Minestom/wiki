...
It is worth reviewing the [Adventure API](/feature/adventure) before this, because these systems depend heavily on `Component`.
...
They are a wrapper around `Advancement`, so you do not need to create any advancements to use them, just a `Notification`. See the [Advancements](/feature/advancements) page for more information on advancements.
